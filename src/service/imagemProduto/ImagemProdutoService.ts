...
export function obterImagens(produtoId: string) {
  return httpClient.get<ImagemProdutoResponseDto[]>(`/produto/${produtoId}/imagens`)
}

export function excluirImagens(produtoId: string, payload: DeleteManyImagensDto) {
...
export function excluirImagem(produtoId: string, id: string) {
